...
(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		_createDirButtons: function () {
			...
			$('<button class="pogoSlider-dir-btn pogoSlider-dir-btn--prev"><i class="fa fa-angle-left"></i>P R E V</button>').appendTo(self.$element).on('click', function () {
				self.prevSlide();
			});

			$('<button class="pogoSlider-dir-btn pogoSlider-dir-btn--next">N E X T<i class="fa fa-angle-right"></i></button>').appendTo(self.$element).on('click', function () {
				self.nextSlide();
			});
